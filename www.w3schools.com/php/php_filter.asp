...
<h1>PHP <span class="color_h1">Filters</span></h1>
<div class="w3-clear nextprev">
<a class="w3-left w3-btn" href="php_sessions.asp">&#10094; Previous</a>
<a class="w3-right w3-btn" href="php_filter_advanced.asp">Next &#10095;</a>
</div>
<hr>
...
<p>The <code class="w3-codespan">filter_list()</code> function can be used to list what the PHP filter extension offers: </p>
...
<a target="_blank" class="w3-btn w3-margin-bottom" href="showphp.asp?filename=demo_filter1">Run example &raquo;</a>
</div>
<hr>
...
<p>The following example uses the <code class="w3-codespan">filter_var()</code> function to remove all HTML tags 
from a string:</p>
...
<a target="_blank" class="w3-btn w3-margin-bottom" href="showphp.asp?filename=demo_filter2">Run example &raquo;</a>
</div>
...
<p>The following example uses the <code class="w3-codespan">filter_var()</code> function to check if the variable $int 
is an integer. If $int is an integer,
the output of the code below will be: "Integer is valid". If $int is not an integer, 
the output will be: "Integer is not valid":</p>
...
<a target="_blank" class="w3-btn w3-margin-bottom" href="showphp.asp?filename=demo_filter3">Run example &raquo;</a>
</div>

<h3>Tip: filter_var() and Problem With 0</h3>
<p>In the example above, if $int was set to 0, the function above will return 
"Integer is not valid". To solve this problem, use the code below:</p>
...
<a target="_blank" class="w3-btn w3-margin-bottom" href="showphp.asp?filename=demo_filter4">Run example &raquo;</a>
</div>
...
<p>The following example uses the <code class="w3-codespan">filter_var()</code> function to check if the variable $ip 
is a valid IP address:</p>
...
<a target="_blank" class="w3-btn w3-margin-bottom" href="showphp.asp?filename=demo_filter5">Run example &raquo;</a>
</div>
...
<p>The following example uses the <code class="w3-codespan">filter_var()</code> function to first remove all 
illegal characters from the $email variable, then check if it 
is a valid email address:</p>

<div class="w3-example">
<h3>Example</h3>
<div class="w3-code htmlHigh notranslate">
 &lt;?php<br>$email = "john.doe@example.com";<br><br>// Remove all illegal characters from email<br>$email = filter_var($email, FILTER_SANITIZE_EMAIL);<br><br>// Validate e-mail<br>if (!filter_var($email, FILTER_VALIDATE_EMAIL) === false) {<br>&nbsp;&nbsp;&nbsp; echo("$email is a valid email address");<br>} else {<br>&nbsp;&nbsp;&nbsp; echo("$email is not a valid email address");<br>}<br>?&gt;
</div>
<a target="_blank" class="w3-btn w3-margin-bottom" href="showphp.asp?filename=demo_filter6">Run example &raquo;</a>
</div>
...
<p>The following example uses the <code class="w3-codespan">filter_var()</code> function to first remove all 
illegal characters from a URL, then check if $url 
is a valid URL:</p>
...
<div class="w3-code htmlHigh notranslate">
 ...
 <br>// Remove all illegal characters from a url<br>$url = filter_var($url, FILTER_SANITIZE_URL);<br><br>// Validate url<br>if (!filter_var($url, FILTER_VALIDATE_URL) === false) {<br>&nbsp;&nbsp;&nbsp; echo("$url is a valid URL");<br>} else {<br>&nbsp;&nbsp;&nbsp; echo("$url is not a valid URL");<br>}<br>?&gt;
</div>
<a target="_blank" class="w3-btn w3-margin-bottom" href="showphp.asp?filename=demo_filter7">Run example &raquo;</a>
</div>

<hr>

<h2>Complete PHP Filter Reference</h2>
<p>For a complete reference of all filter functions, go to our complete
