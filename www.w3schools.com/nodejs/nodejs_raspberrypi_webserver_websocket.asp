...
<!DOCTYPE html>
<html lang="en-US">
<head><title>Node.js Raspberry Pi Webserver with WebSocket</title>
<meta charset="utf-8">
...
<a class="w3-left w3-btn" href="nodejs_raspberrypi_flowing_leds.asp">&#10094; Previous</a>
<a class="w3-right w3-btn" href="nodejs_raspberrypi_rgb_led_websocket.asp">Next &#10095;</a>
</div>
<hr>
<div class="w3-row">
...
<p>webserver.js:</p>
  <div class="w3-code notranslate jsHigh">
    ...
    create server with function handler()<br>var fs = require('fs'); //require filesystem module<br><br>http.listen(8080); //listen to port 8080<br><br>
    function handler (req, res) { //create server<br>&nbsp;&nbsp;fs.readFile(__dirname + '/public/index.html', function(err, data) { //read 
    file index.html in public folder<br>&nbsp;&nbsp;&nbsp; if (err) 
    {<br>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; res.writeHead(404, 
    {'Content-Type': 'text/html'}); //display 404 on error<br>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; 
    return res.end(&quot;404 Not Found&quot;);<br>&nbsp;&nbsp;&nbsp; } <br>&nbsp;&nbsp;&nbsp; res.writeHead(200, {'Content-Type': 'text/html'}); //write HTML<br>&nbsp;&nbsp;&nbsp; res.write(data); //write data from index.html<br>&nbsp;&nbsp;&nbsp; 
    return res.end();<br>&nbsp;&nbsp;});<br>}</div>
</div>
...
<p>index.html:</p>
  <div class="w3-code notranslate jsHigh">
    &lt;!DOCTYPE html&gt;<br>&lt;html&gt;<br>&lt;body&gt;<br>&lt;h1&gt;Control LED light&lt;/h1&gt;<br>&lt;input 
    id=&quot;light&quot; type=&quot;checkbox&quot;&gt;LED<br>&lt;/body&gt;<br>&lt;/html&gt;</div>
</div>
...
<p>index.html:</p>
  <div class="w3-code notranslate jsHigh">
    &lt;!DOCTYPE html&gt;<br>&lt;html&gt;<br>&lt;body&gt;<br>&lt;h1&gt;Control LED 
    light&lt;/h1&gt;<br>&lt;p&gt;&lt;input type=&quot;checkbox&quot; id=&quot;light&quot;&gt;&lt;/p&gt;<br>&lt;script src=&quot;https://cdnjs.cloudflare.com/ajax/libs/socket.io/2.0.3/socket.io.js&quot;&gt;&lt;/script&gt; 
    &lt;!-- include socket.io client side script --&gt;<br>&lt;script&gt;<br>var socket = io(); 
    //load socket.io-client and connect to the host that serves the page<br>
    window.addEventListener(&quot;load&quot;, function(){ //when page loads<br>&nbsp; var 
    lightbox = document.getElementById(&quot;light&quot;); <br>&nbsp; 
    lightbox.addEventListener(&quot;change&quot;, function() { //add event listener for 
    when checkbox changes<br>&nbsp;&nbsp;&nbsp; socket.emit(&quot;light&quot;, Number(this.checked)); 
    //send button status to server (as 1 or 0)<br>&nbsp; });<br>});<br>socket.on('light', 
    function (data) { //get button status from client<br>&nbsp; 
    document.getElementById(&quot;light&quot;).checked = data; //change checkbox according 
    to push button on Raspberry Pi<br>&nbsp; socket.emit(&quot;light&quot;, data); //send 
    push button status to back to server<br>});<br>&lt;/script&gt;<br>&lt;/html&gt;<br>&lt;/body&gt;<br>
    &lt;/html&gt;</div>
</div>
...
<p>webserver.js:</p>
  <div class="w3-code notranslate jsHigh">
    var http = require('http').createServer(handler); //require http server, and 
    create server with function handler()<br>var fs = require('fs'); //require filesystem module<br>var io 
    = require('socket.io')(http) //require socket.io module and pass the http 
    object (server)<br><br>http.listen(8080); //listen to port 8080<br><br>
    function handler (req, res) { //create server<br>&nbsp; fs.readFile(__dirname + '/public/index.html', function(err, data) { //read 
    file index.html in public folder<br>&nbsp;&nbsp;&nbsp; if (err) 
    {<br>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; res.writeHead(404, 
    {'Content-Type': 'text/html'}); //display 404 on error<br>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; 
    return res.end(&quot;404 Not Found&quot;);<br>&nbsp;&nbsp;&nbsp; } <br>&nbsp;&nbsp;&nbsp; res.writeHead(200, {'Content-Type': 'text/html'}); //write HTML<br>&nbsp;&nbsp;&nbsp; res.write(data); //write data from index.html<br>&nbsp;&nbsp;&nbsp; 
    return res.end();<br>&nbsp; });<br>}<br><br>
    io.sockets.on('connection', function (socket) {// WebSocket Connection<br>&nbsp; 
    var lightvalue = 0; //static variable for current status<br>&nbsp; socket.on('light', 
    function(data) { //get light switch status from client<br>&nbsp;&nbsp;&nbsp; 
    lightvalue = data;<br>&nbsp;&nbsp;&nbsp; if (lightvalue) {<br>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; 
    console.log(lightvalue); //turn LED on or off, for now we will just show it 
    in console.log<br>&nbsp;&nbsp;&nbsp; }<br>&nbsp; });<br>});</div>
</div>
...
<p>webserver.js:</p>
  <div class="w3-code notranslate jsHigh">
    ...
    create server with function handler()<br>var fs = require('fs'); //require filesystem module<br>var io 
    = require('socket.io')(http) //require socket.io module and pass the http 
    object (server)<br>var Gpio = require('onoff').Gpio; //include onoff to 
    interact with the GPIO<br>var LED = new Gpio(4, 'out'); //use GPIO pin 4 as 
    output<br>var pushButton = new Gpio(17, 'in', 'both'); //use GPIO pin 17 as 
    input, and 'both' button presses, and releases should be handled<br><br>
    http.listen(8080); //listen to port 8080<br><br>function handler (req, res) 
    { //create server<br>&nbsp; fs.readFile(__dirname 
    + '/public/index.html', function(err, data) { //read file index.html in 
    public folder<br>&nbsp;&nbsp;&nbsp; if (err) {<br>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; res.writeHead(404, {'Content-Type': 'text/html'}); //display 404 on error<br>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; 
    return res.end(&quot;404 Not Found&quot;);<br>&nbsp;&nbsp;&nbsp; } <br>&nbsp;&nbsp;&nbsp; res.writeHead(200, {'Content-Type': 'text/html'}); //write HTML<br>&nbsp;&nbsp;&nbsp; res.write(data); //write data from index.html<br>&nbsp;&nbsp;&nbsp; 
    return res.end();<br>&nbsp; });<br>}<br><br>
    io.sockets.on('connection', function (socket) {// WebSocket Connection<br>&nbsp; 
    var lightvalue = 0; //static variable for current status<br>&nbsp; 
    pushButton.watch(function (err, value) { //Watch for hardware interrupts on 
    pushButton<br>&nbsp;&nbsp;&nbsp; if (err) { //if an error<br>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; 
    console.error('There was an error', err); //output error message to console<br>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; 
    return;<br>&nbsp;&nbsp;&nbsp; }<br>&nbsp;&nbsp;&nbsp; lightvalue = value;<br>&nbsp;&nbsp;&nbsp; 
    socket.emit('light', lightvalue); //send button status to client<br>&nbsp; 
    });<br>&nbsp; socket.on('light', function(data) { //get light switch status 
    from client<br>&nbsp;&nbsp;&nbsp; lightvalue = data;<br>&nbsp;&nbsp;&nbsp; 
    if (lightvalue != LED.readSync()) { //only change LED if status has changed<br>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; 
    LED.writeSync(lightvalue); //turn LED on or off<br>&nbsp;&nbsp;&nbsp; }<br>&nbsp; 
    });<br>});<br><br>process.on('SIGINT', function () { //on ctrl+c<br>&nbsp; 
    LED.writeSync(0); // Turn LED off<br>&nbsp; LED.unexport(); // Unexport LED 
    GPIO to free resources<br>&nbsp; pushButton.unexport(); // Unexport Button 
    GPIO to free resources<br>&nbsp; process.exit(); //exit completely<br>});</div>
</div>
