...
<!DOCTYPE html>
<html lang="en-US">
<head><title>Node.js Raspberry Pi LED and Pushbutton</title>
<meta charset="utf-8">
...
<a class="w3-left w3-btn" href="nodejs_raspberrypi_blinking_led.asp">&#10094; Previous</a>
<a class="w3-right w3-btn" href="nodejs_raspberrypi_flowing_leds.asp">Next &#10095;</a>
</div>
<hr>
...
<div class="w3-code notranslate jsHigh">
  var
  Gpio = require('onoff').Gpio; //include onoff to interact with the GPIO<br>var 
  LED = new Gpio(4, 'out'); //use GPIO pin 4 as output<br>var pushButton = new 
  Gpio(17, 'in', 'both'); //use GPIO pin 17 as input, and 'both' button presses, 
  and releases should be handled<br><br>pushButton.watch(function (err, value) { 
  //Watch for hardware interrupts on pushButton GPIO, specify callback function<br>&nbsp; 
  if (err) { //if an error<br>&nbsp;&nbsp;&nbsp; console.error('There was an 
  error', err); //output error message to console<br>&nbsp; return;<br>&nbsp; }<br>&nbsp; 
  LED.writeSync(value); //turn LED on or off depending on the button state (0 or 
  1)<br>});<br><br>
  function unexportOnClose() { //function to run when exiting program<br>&nbsp; 
  LED.writeSync(0); // Turn LED off<br>&nbsp; LED.unexport(); // Unexport LED 
  GPIO to free resources<br>&nbsp; pushButton.unexport(); // Unexport Button 
  GPIO to free resources<br>};<br><br>process.on('SIGINT', unexportOnClose); //function to 
  run when user closes using ctrl+c</div>
</div>
