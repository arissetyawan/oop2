...
<!DOCTYPE html>
<html lang="en-US">
<head><title>Node.js Raspberry Pi Blinking LED</title>
<meta charset="utf-8">
...
<a class="w3-left w3-btn" href="nodejs_raspberrypi_gpio_intro.asp">&#10094; Previous</a>
<a class="w3-right w3-btn" href="nodejs_raspberrypi_led_pushbutton.asp">Next &#10095;</a>
</div>
<hr>
...
<div class="w3-code notranslate jsHigh">
  var Gpio = require('onoff').Gpio; //include onoff to interact with the GPIO<br>var LED = new Gpio(4, 'out'); 
  //use GPIO pin 4, and specify that it is output<br>var 
  blinkInterval = setInterval(blinkLED, 250); //run the blinkLED function every 
  250ms<br><br>function blinkLED() { //function to start blinking<br>&nbsp; if (LED.readSync() === 
  0) { //check the pin state, if the state is 0 (or off)<br>&nbsp;&nbsp;&nbsp; LED.writeSync(1); 
  //set pin state to 1 (turn LED on)<br>&nbsp; } else {<br>&nbsp;&nbsp;&nbsp; LED.writeSync(0); 
  //set pin state to 0 (turn LED off)<br>&nbsp; }<br>}<br><br>function endBlink() 
  { //function to stop blinking<br>&nbsp; clearInterval(blinkInterval); // Stop blink intervals<br>&nbsp; 
  LED.writeSync(0); // Turn LED off<br>&nbsp; LED.unexport(); // Unexport GPIO 
  to free resources<br>}<br><br>setTimeout(endBlink, 5000); //stop blinking 
  after 5 seconds</div>
</div>
